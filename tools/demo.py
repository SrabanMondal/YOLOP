...
## ADAS FUNCTIONALITY
# ADD THESE IMPORTS AND CLASSES (keep your existing ones)
...
class LaneCurvature:
    ...
    def fit_centerline(self, da_mask):
        h, w = da_mask.shape
        y, x = np.where(da_mask == 1)
        if len(y) < 100:
            return None
        ...
        if self.avg_fit is None:
            ...
            self.avg_fit = self.alpha * self.avg_fit + (1 - self.alpha) * fit

        return self.avg_fit

    def generate_points(self, fit, h):
...
def detect(cfg, opt):
    ...
    for i, (path, img, img_det, vid_cap, shapes) in tqdm(enumerate(dataset), total=len(dataset)):
        # img = Tensor (resized, padded). 
        # img_det = Original Image (Or unpadded raw frame).
        ...
        _, _, height, width = img.shape # Tensor shape
        ...
        # if len(xs) > 0:
        #     deviation = np.mean(xs) - (da_seg_mask.shape[1] / 2)
        #     if deviation > 20: steering = "RIGHT"
        #     elif deviation < -20: steering = "LEFT"
        
        # Apply segmentation mask to img_det
        # NOTE: show_seg_result might resize img_det, so we must grab shape AFTER this line
        # img_det = show_seg_result(img_det, (da_seg_mask, ll_seg_mask), _, _, is_demo=True)
        
        # ----------------------------------------------------
        # 1. Use show_seg_result (keeps perfect alignment)
        # ----------------------------------------------------
        img_det = show_seg_result(img_det, (da_seg_mask, ll_seg_mask), _, _, is_demo=True)

        # Get current frame size (after show_seg_result — now guaranteed correct)
        h_draw, w_draw = img_det.shape[:2]

        # ----------------------------------------------------
        # 2. Extract drivable mask AGAIN — but now correctly sized!
        # ----------------------------------------------------
        # Recompute da_seg_mask using proper scaling (same as before)
        da_predict = da_seg_out[:, :, int(pad_h):(height-int(pad_h)), int(pad_w):(width-int(pad_w))]
        da_seg_mask = torch.nn.functional.interpolate(da_predict, scale_factor=int(1/ratio), mode='bilinear')
        _, da_seg_mask = torch.max(da_seg_mask, 1)
        da_seg_mask = da_seg_mask.int().squeeze().cpu().numpy()

        # CRITICAL: Resize mask to CURRENT drawing frame (h_draw, w_draw)
        # This is now SAFE because show_seg_result already resized the image correctly
        da_seg_mask = cv2.resize(da_seg_mask, (w_draw, h_draw), interpolation=cv2.INTER_NEAREST)

        # ----------------------------------------------------
        # 3. Smooth Lane Centerline + Kalman Steering
        # ----------------------------------------------------
        global lane_curve, kalman  # Initialize outside loop
        if 'lane_curve' not in globals():
            lane_curve = LaneCurvature(alpha=0.85)
            kalman = KalmanSmoother()

        fit = lane_curve.fit_centerline(da_seg_mask)
        raw_deviation = 0
        if fit is not None:
            bottom_x = fit[0] * (h_draw - 1)**2 + fit[1] * (h_draw - 1) + fit[2]
            raw_deviation = bottom_x - w_draw / 2

        smooth_deviation = kalman.update(raw_deviation)
        ...
        # ----------------------------------------------------
        # 4. Draw Smooth Red Centerline
        # ----------------------------------------------------
        if fit is not None:
            pts = lane_curve.generate_points(fit, h_draw)
            if pts is not None:
                cv2.polylines(img_det, [pts], isClosed=False, color=(0, 0, 255), thickness=6)
                # Optional: draw a glowing effect
                cv2.polylines(img_det, [pts], isClosed=False, color=(0, 100, 255), thickness=12)

        # ----------------------------------------------------
        # 5. Draw 3D Arrow using SMOOTHED deviation
        # ----------------------------------------------------
        draw_3d_arrow(img_det, smooth_deviation)

        # ----------------------------------------------------
        # CRITICAL FIX: GET ROI ON ACTUAL DRAWING FRAME
        # ----------------------------------------------------
        # We grab the shape from img_det right here. 
        # This guarantees that 'h_draw' and 'w_draw' match the pixels we are about to paint on.
        h_draw, w_draw = img_det.shape[:2]
        
        # 1. Calculate ROI Polygon using ACTUAL dimensions
        roi_poly = get_roi_polygon(h_draw, w_draw)
        
        # 2. Run YOLO (Pass img_det so YOLO scales boxes to this image automatically)
        # By passing the same image we draw on, Ultralytics handles the resizing internally.
        rgb_frame = cv2.cvtColor(img_det, cv2.COLOR_BGR2RGB)
        results = model_det.predict(rgb_frame, conf=0.5, verbose=False)
        
        # Boxes are now in [x1, y1, x2, y2] relative to h_draw, w_draw
        ...
        # 3. Filter Boxes against ROI
        for (xyxy, conf, cls_id) in zip(boxes, confs, classes):
            # We pass h_draw, w_draw to ensure intersection mask matches image size
        ...
        # if len(xs) > 0: draw_3d_arrow(img_det, deviation)
